...
import { getCardFromCardId } from '../deck';
...
function findCard(player, cardId: string) {
    const card = getCardFromCardId(cardId);
    return player.cards.find((c) => c.id === card.id);
}
...
export function throwCard(game: GameEngine, event: GameEvent): EventResult {
    ...
    const card = findCard(player, event.data.cardId);
    if (!card) {
